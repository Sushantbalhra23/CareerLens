...
const suggestions = [
  ...
  "Plan 3 month DSA schedule",
  "Add Docker to my roadmap",
  "Review my progress"
]

export function AIPanelFull({
  tasks,
  setTasks,
}: {
  tasks: any[]
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiMessage = {
        role: "assistant",
        content: res.data.roadmapText || res.data.reply || "Done ✅"
      }
    ...
    <div className="flex flex-col h-full">
        ...
        {chatHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Ask me anything about your roadmap, skills, or planning strategy
            </p>
          ...
          chatHistory.map((msg, index) => (
            <div
              key={index}
              className={`p-3 rounded-lg text-sm max-w-[70%] ${msg.role === "user"
      ...
      <div className="relative">
        ...
        <input
          ...
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Ask CareerLens AI anything..."
          className="h-12 w-full rounded-xl border border-input bg-background pl-4 pr-24 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
          onKeyDown={(e) => {
            if (e.key === "Enter") handleSend()
          }}
        />
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          {suggestions.map((suggestion) => (
            <button
              key={suggestion}
              onClick={() => setInput(suggestion)}
              className="rounded-full border px-3 py-1.5 text-xs text-muted-foreground hover:border-primary hover:text-primary"
            >
              {suggestion}
            </button>
          ))}
